...
Confidence_emotions = ["Calmness","Concentration","Determination","Interest","Triumph","Excitement"]
...
def get_emotional_maps(events):
    # Stores the highlight-worthy emotional segments of the user during the conversation - positive and negatives.
    # Note that agent here is the simulated user and user here is the sales agent being trained.
    emotional_segments_user = {}
    emotional_segments_agent = {}
    interruptions = []
    event_count = 0
    ...
    for event in events[1:]:
        ...
        if event_type == "USER_MESSAGE":
            try:
                emotions = dict(sorted(json.loads(event["emotion_features"]).items(), key=lambda item: item[1], reverse=True))
                top_emotion = max(emotions.items(), key=lambda x: x[1])
                if top_emotion[0] not in emotional_segments_user:
                    emotional_segments_user[top_emotion[0]] = 1
                else:
                    emotional_segments_user[top_emotion[0]] += 1
            except Exception as e:
                logging.error(f"Error processing USER_MESSAGE: {e}")

        elif event_type == "AGENT_MESSAGE":
            try:
                emotions = dict(sorted(json.loads(event["emotion_features"]).items(), key=lambda item: item[1], reverse=True))
                top_emotion = max(emotions.items(), key=lambda x: x[1])
                if top_emotion[0] not in emotional_segments_agent:
                    emotional_segments_agent[top_emotion[0]] = 1
                else:
                    emotional_segments_agent[top_emotion[0]] += 1
            except Exception as e:
                logging.error(f"Error processing AGENT_MESSAGE: {e}")

        elif event_type == "USER_INTERRUPTION":
            interruptions.append((event["timestamp"], event["message"]))
            event_count += 1

    logging.info(f"Emotional segments (Agent): {emotional_segments_agent}")
    logging.info(f"Emotional segments (User): {emotional_segments_user}")

    return emotional_segments_agent, emotional_segments_user, interruptions
...
def get_emotional_maps(events):
    ...
    # this store the highlight worthy emotional segments of the user during the conversation - positive and negatives
    # note that agent here is the simulated user and user here is the sales agent being trained
    emotional_segments_user = {}
    emotional_segments_agent = {}
    interruptions = []
    ...
    for event in events[1:]:
        match event["type"]:
            ...
            case "USER_INTERRUPTION":
                interruptions.append((event["timestamp"],event["message"]))
                event_count+=1
    #logging.info(f"emotional segments {emotional_segments_user}")
    #print(f"emotional segments {emotional_segments_user}")
    return emotional_segments_agent, emotional_segments_user, interruptions

async def getPreliminaryReport(chatData):
    payload = chatData
    
    # freq map of emotions through the conversation
    
    try:
        emotions_agent, emotions_user, interruptions = get_emotional_maps(payload["data"]["events_page"])
    except Exception as e:
        print(f"An error occurred in emotion parsing call: {e}")
        return None
...
def getReportDataFromHumeEVI(chatData):
    payload = json.loads(chatData)

    #logger.info(payload)
    interruptions = []
    
    #1 emotional segments of conversations
    ...
    # sequence of emotions will be stored here
    Sequence_of_emotions = {}
    current_sequence = 0
    ...
    temp_seq = []
    prev = "None"
    for event in payload["data"]["events_page"]:
        match event["type"]:
            case "USER_MESSAGE":
                emotions = dict(sorted(event["emotion_features"].items(), key=lambda item: item[1],reverse=True))
                if cur == "agent":
                    # now compute avg of prev list till now and append to seq
                    Avg_emotion = get_avg_emotion(temp_seq)
                    temp_seq  = []
                    Sequence_of_emotions[current_sequence] = temp_seq
                    current_sequence+=1
                else:
                    temp_seq.append((event["id"],emotions))

                # putting a list here with [text and {top 3 emotions}]
                emotional_segments_user.append([event["message_text"],{emotions:emotions[i] for i in emotions[:3]}])
                event_count+=1
                cur = "user"
                break
            case "AGENT_MESSAGE":
                ...
                emotional_segments_agent.append([event["message_text"],{emotions:emotions[i] for i in emotions[:3]}])
                event_count+=1

                cur = "agent"
                # if agent (user simulation) gave a positive sentiment, did agent capitalize on it
                break
            case "USER_INTERRUPTION":
                interruptions.append((event["timestamp"],event_count))
                event_count+=1
                break
            
            

    # this stores agents_emotional_roadmap - mainly highlighting rude, indecisive, lack of confidence and more
    # make a freq table for emotions across segments highlighting positive with green and negative with red
    Net_Emotions_User = All_emotions.copy()
    Net_Emotions_Agent = All_emotions.copy()
    for e in emotional_segments_user:
        top_emotion = e[1][0]
        Net_Emotions_User[top_emotion] +=1

    for e in emotional_segments_agent:
        top_emotion = e[1][0]
        Net_Emotions_Agent[top_emotion] +=1

    # send this and construct
    ...
    # interruptions - already constructed,send back
    # confidence scores - 
    confidence_score = sum(Net_Emotions_User[k] for k in Confidence_emotions) / sum(emotional_segments_user.values())
    ...
    # TO - DO
    # map emotion of user to agent and see where he f'ed up
    # emotions should match if user emotions positive or be complimentary  if theyre negative
    # use Sequence_of_emotions from above for this


    # 3. script intelligence
    # catch phrases for users for positive emotions
    # catch phrases to avoid during negative emotions
    Positive_Phrases = []
    Negative_Phrases = []
    for segment in emotional_segments_agent:
        if float(segment[1][0].value) > 0.15 and segment[1][0].key in Positive_emotions:
            Positive_Phrases.append((segment[0], segment[1][0].key))
        elif float(segment[1][0].value) > 0.15 and segment[1][0].key in Negative_emotions:
            Negative_Phrases.append((segment[0], segment[1][0].key))
    # send this to llm to get intelligent phrases etc to use and ones not to use
    ...
    # quantify objections raises
    # quantify responses given
    # quantify missed opportunities - when agent should have capitalized on positive emotions
    """
    LLM ask for objectons and responses given across conversation
    ...
    # Product correctness and concerns
    # agent product correctness
    # agent compliance correctness
    # agent to do's to get better
    
    
    
    
    # below section is for real calls not simulations - or can we ?
    ...
    # next steps
    # highlight positive emotions and match it to use it to tailor follow ups
    # highlight curiosity and uncertainty to find where to offer more clarity

    # detect signals across emotions and ask llm to summarize
    # what worked 
    # what didnt work
    ...
    # account insights
    # insights for managers on what works, what doesn't work
    # sale concerns, sale positives, ischampion or not


    # product insights
    # follow ups with product teams


    # marketing insights
    # messaging thats resonating
